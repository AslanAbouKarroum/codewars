...
function phone(strng, num) {
    ...
    if(arr.length>1){
        ...
        arr = arr.join('').split('');
        let name = arr.slice(arr.findIndex(e=>e=='<')+1,arr.findIndex(e=>e=='>')).join('');
        arr.splice(arr.findIndex(e=>e=='<'),name.length+2);
        arr.splice(arr.findIndex(e=>e=='+'),num.length+2);
        let address = arr.join('').replace(/\s+/g,' ').replace('_',' ').replace(/[^\w\s-]/g,'').trim();
