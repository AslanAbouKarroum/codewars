...
function lengthSupUK(n, k) {
  return sequence(n).filter(i => i >= k).length;
}

function comp(n) {
  return sequence(n).slice(1).reduce((total, n, i, seq) => total + (n < seq[i - 1] ? 1 : 0), 0);
}

function u(n) {
  return sequence(n).pop();
}
...
function sequence(n) {
  ...
  for (let i = 3; i <= n; i++) {
    seq.push(seq[i - seq[i - 1]] + seq[i - seq[i - 2]]);
  }
