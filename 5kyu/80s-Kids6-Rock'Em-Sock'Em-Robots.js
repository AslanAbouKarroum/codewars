...
function fight(robot1, robot2, tactics) {
    while(robot1.health && robot2.health && (robot1['tactics'].length>0 || robot2['tactics'].length>0)){
        if(robot1.speed >= robot2.speed){
            if(robot1['tactics'].length>0) robot2.health -= tactics[robot1['tactics'].shift()];
            if(robot2.health<=0) break;
            if(robot2['tactics'].length>0) robot1.health -= tactics[robot2['tactics'].shift()];
    ...
    return robot1.health > robot2.health ? `${robot1.name} has won the fight.`  : (robot2.health > robot1.health  ? `${robot2.name} has won the fight.` : "The fight was a draw.")
};

// Solution
function fight(r1, r2, tactics) {
    const robots = r1.speed >= r2.speed  ? [r1,r2] : [r2, r1];
    let tactic = 0;
    
    const fighting = () => r1.health > 0 && r2.health > 0 && 
                           (r1.tactics.length > tactic || r2.tactics.length > tactic);
    ...
    return r1.health > r2.health ? `${r1.name} has won the fight.` :
           r2.health > r1.health ? `${r2.name} has won the fight.` :
