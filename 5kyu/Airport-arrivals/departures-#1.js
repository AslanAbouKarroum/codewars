...
// My Solution
let table = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ?!@#&()|<>.:=-+*/0123456789';
console.log(table.length);
var flapDisplay = function(lines, rotors){
    let input = lines.map(e=>e.split(''));
    let result = [];
    for(let j=0;j<input.length;j++){
        result.push(input[j].map((e,i)=> table[(table.split('').findIndex(el=>el==e)+rotors[j].slice(0,i+1).reduce((acc,e)=>acc+e))%54]).join(''))
    };
    return result;
};
...
var flipChar = function(char, flips) {
    return ALPHABET[(ALPHABET.indexOf(char) + flips) % ALPHABET.length];  // ALPHABET is a preloded constant which is a string
  }
