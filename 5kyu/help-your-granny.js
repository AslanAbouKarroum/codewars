...
function tour(friends, fTowns, distTable) {
    ...
    friends.forEach(function(friend) {
      var hometown = findTown(friend);
      if (hometown) {
        var dist = findDistance(hometown);
        if (dist) dists.push(dist);
      }
    });
    // Process it
    return Math.floor(dists.reduce(sumLegs) + dists[dists.length - 1]);
    // Helper functions
    function findTown(person) {
      for (var i = 0; i < fTowns.length; i ++) {
        if (fTowns[i][0] === person) return fTowns[i][1];
      }
    }
    function findDistance(town) {
      for (var i = 0; i < distTable.length; i += 2) {
        if (distTable[i] === town) return distTable[i + 1];
      }
    }
    function sumLegs(acc, el, ind, arr) {
      return acc += Math.sqrt(Math.pow(el, 2) - Math.pow(arr[ind - 1], 2));
    }
  }