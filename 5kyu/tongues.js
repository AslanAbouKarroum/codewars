...
function tongues(code) {
    return code.split(' ').map(e=>e.split('').map(el=>{if(el.charCodeAt(0)>=65&&el.charCodeAt(0)<=90){return code_table[el.toLowerCase()].toUpperCase()}else{return code_table[el]||el}}).join('')).join(' ');
}
let code_table={
    'a' : 'e',
    'i' : 'o',
