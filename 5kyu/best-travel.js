...
function chooseBestSum(t, k, ls){
    ...
    let arr = combinations(ls,k);
    arr =arr.map(e=>e.reduce((a,b)=>a+b));
    arr.sort((a,b)=>b-a);
    return arr.find(e=>e<=t) || null;
};

function combinations(arr, length) {
    const results = [];
    if (length > arr.length) return results; // Handle invalid lengths
    function backtrack(current, start) {
      if (current.length === length) {
        results.push(current.slice()); // Deep copy to avoid reference issues
        return;
      };
      for (let i = start; i < arr.length; i++) {
        current.push(arr[i]);
        backtrack(current, i + 1); // Avoid duplicates by starting from i + 1
