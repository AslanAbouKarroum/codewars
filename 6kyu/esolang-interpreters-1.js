...
function myFirstInterpreter(code) {
    return code.split('').filter(e=>e=='+'|| e=='.').join('').split('.').map(e=>e.length).map((e,i,ar)=>{let ac = 0;for(let j=0;j<i;j++){ ac=ac+ar[j]}return ac+e||e}).map(e=>e%256).map(e=>(String.fromCharCode(e))).join('').slice(0,-1)
};
